...
This is solutions for problems on the second edition of \textit{Category for the Working Mathematician} by S. Mac Lane. Problems are placed in the first line of each sections. They may be modified from the original text for simplicity, clarity and some laziness.
...
Let the set of all object in $C$ be the product set $\prod_i C_i$ and $C(a, b) = \prod_i C_i(a_i, b_i)$. Let $\mathrm{id}_C(c)_i = \mathrm{id}_{C_i}(c_i)$ for all $c \in C$. Now, we proof $C$ has a universal property:
...
\textbf{Let A, B and C are small categories. Proof that $\mathbf{Cat}(A \times B, C) \cong \mathbf{Cat}(A, C^B)$ and that they are natural.}

First, we proof there are two bijections $f : \mathbf{Cat}(A \times B, C) \rightarrow \mathbf{Cat}(A, C^B)$ and $g : \mathbf{Cat}(A, C^B) \rightarrow \mathbf{Cat}(A \times B, C)$ such that $f \circ g = g \circ f = \mathrm{id}$. For any $x : A \times B \rightarrow C$, . Let $f(x) = $ and $g(x) = $.

Second, we proof $f$ and $g$ are natural.
...
\textbf{Let A, B and C are categories. Proof that $(A \times B)^C \cong A^C \times B^C$ and $C^{A \times B} \cong (C^B)^A$.}
