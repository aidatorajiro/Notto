...
Let $G, G'$ is a graph of the same set of objects $O$ and the set of arrows $A, A'$. Make a function $D : \mathbf{Grph} \rightarrow \mathbf{Grph}$ from the object function $D_O : O \rightarrow O$ and the arrow function $D_A : A \rightarrow A'$. Define $D_O = \mathrm{id}$, $D_A f : \mathrm{cod} f \rightarrow \mathrm{dom} f$, and $D_A f \circ D_A g = D_A (g \circ f)$ for all $f, g \in A$. The $D_O$ and $D_A$
