...
Solutions for problems on the second edition of \textit{Category for the Working Mathematician} by S. Mac Lane.
...
\begin{center}
    ...
    k \arrow[mapsto, r, swap, "Tk"] & Tk & \arrow[mapsto, l, "t'_0"] t' \arrow[mapsto, r, swap, "t'_1"] & Sh & \arrow[mapsto, l, "Sh"] h
    \end{tikzcd}
\end{center}
...
\begin{center}
    \begin{tikzcd}[row sep=4em, column sep=4em]
     ...
     \arrow[d, "Q'"]\arrow[dll, swap, "P'"]\arrow[drr, "R'"] & &\\
...
% comment: $(f^\mathrm{op})^\mathrm{op} = f$ for all $f \in A$ may be necessary ?
...
\begin{center}
  \begin{tikzcd}[column sep=6em,row sep=6em]
    C_{a, b}                 \arrow{r}{\mathrm{op}}[swap]{\cong} \arrow{rd}{U}[swap]{=}&
    (C^\mathrm{op})_{a, b}   \arrow{r}{U}[swap]{=}&
    (U(C^\mathrm{op}))_{a, b}\\
    &
    (UC)_{a, b}              \arrow{r}{\mathrm{op}}[swap]{\cong}&
    ((UC)^\mathrm{op})_{a, b}
  \end{tikzcd}
\end{center}
...
\begin{center}
  \begin{tikzcd}[column sep=4em,row sep=4em]
    F \arrow[d, dashed, "D'"] & G \arrow[r, "P"] \arrow[rd, "D", swap] & U F \arrow[d, dashed, "U D'"]\\
    E & & U E
  \end{tikzcd}
\end{center}
