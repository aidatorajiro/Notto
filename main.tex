...
Let $FT = S$, where $T : A \times B \rightarrow C$ and $S : A \rightarrow C^B$. First, we make the object function of $S$. Given $a \in A$, we make a subcategory $a \subseteq A$, a category of an object $a$ and an arrow $\mathrm{id}(a)$. Trivially, there is a functor $f : B \rightarrow a \times B$. As $a \times B \subseteq A \times B$, now we define $Sa : B \rightarrow C$, $Sa = T \circ f$. Second, we make the arrow function of $S$. Let $Sa = \tau$, where $a : a_1 \rightarrow a_2$ is an arrow in $A$, $\tau : S a_1\ \dot{\rightarrow}\ S a_2$, $\tau b = T(\langle a, \mathrm{id}(b) \rangle)$. Let $g : b_1 \rightarrow b_2$ is an arrow in $B$. Then $Sa_2g(\tau b_1(Sa_1b_1)) = Sa_2b_2 = \tau b_2(Sa_1g (Sa_1b_1))$. Thus $\tau$ is natural.
...
Finally, we prove that for all $T_1,\ T_2 : A \times B \rightarrow C$, if $FT_1 = FT_2$, then $T_1 = T_2$. Let $FT_1 = FT_2$ and $n \in \{1,\ 2\}$. First, for all $a \in A$, $b \in B$, $FT_nab = T_n(\langle a, b \rangle)$. Thus $T_1 = T_2$ for the object functions. Second, for every object $a \in A$, arrow $b$ in $B$, $FT_nab = T_nab$. And for every arrow $a$ in $A$, object $b \in B$, $FT_nab = T_n(\langle a, \mathrm{id}(b) \rangle)$. Thus $T_1 = T_2$ for the arrow functions.
